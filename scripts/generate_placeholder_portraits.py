...
def write_portrait(path: str, svg_body: str) -> bool:
    """Write ``svg_body`` to ``path`` if it differs from the current file.

    Returning ``True`` when a file changes keeps the generator idempotent and
    prevents unnecessary diffs that can surface as merge conflicts when
    multiple contributors regenerate the same assets.
    """

    new_contents = svg_body + "\n"
    try:
        with open(path, "r", encoding="utf-8") as existing:
            if existing.read() == new_contents:
                return False
...
def main() -> None:
    ...
    for slug, name, lines, base_color in FIGURES:
        ...
        if write_portrait(path, svg):
            changed += 1
    print(
        f"Ensured {len(FIGURES)} SVG portraits in {output_dir} (updated {changed})."
    )
